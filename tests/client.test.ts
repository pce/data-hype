...
import { fetchJson, getCsrfToken } from '../src/client';
...
describe('client helpers', () => {
  ...
  describe('fetchJson', () => {
    ...
    it('handles invalid JSON gracefully and returns ok: false with status', async () => {
      ...
      const result = await fetchJson('/bad-json');
    ...
    it('returns ok:false and error when fetch rejects', async () => {
      ...
      const result = await fetchJson('/network-error');
      expect(result).toHaveProperty('ok', false);
  ...
  describe('getCsrfToken', () => {
    it('returns token string when endpoint responds with ok and csrfToken', async () => {
      mockFetch.mockResolvedValueOnce(
        new Response(JSON.stringify({ ok: true, csrfToken: 'abc123' }), { status: 200 })
      );

      const token = await getCsrfToken('/csrf-token');
